...
PATS = "ff3dnod3hagor6czaxfmzrlujgr446ps44v6jufzjeevrxuron6q"
## organization name
...
column_names = ['project_name','repository_name','branch_name','repository_id','last_commit_id','haveActivePullRequests','committer_name','last_commit_date','comment','last_commit_link']
...
if response.status_code == 200 :
  ...
  for idx,repo in enumerate(repository_ids) :
    ...
    response = requests.get(url,headers=headers,auth=auth)
    url1 = f"https://dev.azure.com/{organization}/{project}/_apis/git/repositories/{repositoryId}/pullRequests?searchCriteria.status=active&skip=0"
    res = requests.get(url1,headers=headers,auth=auth)
    if res.status_code == 200 :
      isActive = res.json()['count']>0
    # checking if the response is OK or NOT
    if response.status_code == 200 :
      ...
      data = [repo[0],repository_names[idx],name,repo[1],commitId,isActive,committerName,commitDate,comment,commitURL]
      ...
      if response.status_code == 200:
        ...
        comment = res['commit']['comment']
        commitURL = res['commit']['url']
        data = [repo[0],repository_names[idx],name,repo[1],commitId,isActive,authorName,commitDate,comment,commitURL]
        new_df = pd.DataFrame([data],columns=column_names)
        df = pd.concat([df, new_df], axis=0, ignore_index=True)
      else :
        columns = ['project_name','repositoryId']
        data = [repo[0],repositoryId]
